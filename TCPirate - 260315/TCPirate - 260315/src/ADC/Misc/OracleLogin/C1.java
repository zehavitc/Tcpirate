...
public class C1
{
    ...
    {
        System.out.print((char)a((byte)((byte0 & 0xf0) >> 4)));
        System.out.print((char)a((byte)(byte0 & 0xf)));
        System.out.print(" ");
    }
    ...
    public byte[] c(byte abyte0[], byte abyte1[])
    {
        C0 c0 = new C0();
        c0.l(abyte0);
        byte abyte2[] = c0.q(abyte1, false);
        return abyte2;
    }

    private byte[] d(String s1, boolean flag)
    {
        int i1 = s1.length();
    ...
    private static byte f(byte byte0)
    {
        byte byte1;
        if(byte0 >= 97 && byte0 <= 102)
            byte1 = (byte)((byte0 - 97) + 10);
        else
        if(byte0 >= 65 && byte0 <= 70)
            byte1 = (byte)((byte0 - 65) + 10);
        else
    ...
    public byte[] g(String s1, String s2)
    {
        byte abyte0[] = k(s1, s2, true);
        byte abyte1[] = h(abyte0);
        return abyte1;
    }

    private byte[] h(byte abyte0[])
    {
        ...
        C0 c0 = new C0();
        c0.l(abyte3);
        abyte1 = c0.q(abyte0, true);
        c0.l(abyte1);
        abyte2 = c0.q(abyte0, true);
        return abyte2;
    }

    private static byte[] i(byte abyte0[])
    {
        byte abyte1[] = new byte[abyte0.length / 2];
        for(int i1 = 0; i1 < abyte1.length; i1++)
        {
            abyte1[i1] = (byte)(f(abyte0[i1 * 2]) << 4);
            abyte1[i1] |= f(abyte0[i1 * 2 + 1]);
        }

        return abyte1;
    }

    public byte[] j(byte abyte0[], byte abyte1[])
    {
        C0 c0 = new C0();
        byte abyte2[] = i(abyte1);
        c0.d(abyte0, abyte2, 1);
        return abyte2;
    }

    private byte[] k(String s1, String s2, boolean flag)
    {
        int i1 = s1.length();
        int j1 = s2.length();
        int k1 = (i1 + j1) * 2;
        byte abyte0[] = new byte[k1 % 8 <= 0 ? (k1 / 8) * 8 : (k1 / 8 + 1) * 8];
        int l1 = l(s1, abyte0, 0, flag);
        l(s2, abyte0, l1, flag);
        return abyte0;
    }

    private int l(String s1, byte abyte0[], int i1, boolean flag)
    {
        int i2 = s1.length();
        int l1;
        for(l1 = 0; l1 < i2; l1++)
        {
            char c1;
            if(flag && s1.charAt(l1) < '\200')
                c1 = Character.toUpperCase(s1.charAt(l1));
            else
            if(flag)
            {
                int j1 = e(o, s1.charAt(l1));
                c1 = Character.toUpperCase((char)j1);
            } else
            if(s1.charAt(l1) < '\u2000')
                c1 = Character.toUpperCase(s1.charAt(l1));
            else
            ...
            if(c1 < '\200')
                ...
                k1 = e(s, c1);
            abyte0[i1 + l1 * 2] = (byte)(k1 >> 8);
            abyte0[i1 + l1 * 2 + 1] = (byte)k1;
        }

        return i1 + l1 * 2;
    }

    public byte[] m(String s1, String s2, boolean flag)
    {
        byte abyte0[] = k(s1, s2, flag);
        byte abyte1[] = h(abyte0);
        return abyte1;
    }
